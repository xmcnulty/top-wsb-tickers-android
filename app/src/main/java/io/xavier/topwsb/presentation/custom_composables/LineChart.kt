...
fun LineChart(
    ...
    val (lowerVal, upperVal) = remember(key1 = data) {
        Pair(
            data.minByOrNull { it.y },
            data.maxByOrNull { it.y }
        )
    }
    ...
    Canvas(modifier = modifier) {
        ...
        val strokePath = Path().apply {
            ...
            for (i in data.indices) {
                ...
                val leftRatio =
                    (info.y - (lowerVal?.y ?: 0.0)) / ((upperVal?.y ?: 0.0) - (lowerVal?.y ?: 0.0))
                val rightRatio =
                    (nextInfo.y - (lowerVal?.y ?: 0.0)) / ((upperVal?.y ?: 0.0) - (lowerVal?.y ?: 0.0))
        ...
        if (showYLabels) {
            ...
            drawContext.canvas.nativeCanvas.apply {
                drawText(
                    "MAX ${upperVal?.yLabel.orEmpty()}",
                    size.width - 16.dp.toPx(),
                    0 + 8.dp.toPx(),
                    textPaint
                )
                drawText(
                    "MIN ${lowerVal?.yLabel.orEmpty()}",
                    size.width - 16.dp.toPx(),
